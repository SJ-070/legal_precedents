...
with tab2:
    st.header("🔍 판례 검색")
    st.markdown("사건번호(2023도1907, 2017구합53518 등), 판례번호, 날짜, 법원명 등으로 판례를 직접 검색할 수 있습니다.")
    ...
    with col1:
        search_query = st.text_input(
            "검색어 입력",
            placeholder="예: 2023도1907, 대법원, 2024-12-19, 인천공항세관 등",
            key="search_input"
        )
    with col2:
        ...
        if not has_data_files:
            ...
            if results:
                ...
                with st.expander("💡 검색 팁 및 권장사항"):
                    ...
                    **복합 검색 시 유의사항:**
                    - ⚠️ 긴 검색어는 점수 희석 가능
                    - 예: `대법원 2008. 1. 10. 선고 2006두19105 판결` (비권장)
                    - → 이유: 여러 패턴 동시 매칭으로 정확도 저하
    ...
    if not search_button:
        st.info("👆 위의 검색창에 사건번호, 판례번호, 날짜, 법원명 등을 입력하고 검색 버튼을 클릭하세요.")
        ...
        with st.expander("📚 검색 가이드 및 사용 예시", expanded=True):
            ...
            **6. 복합 검색 (간단한 조합만 권장)**
            ...
            ⚠️ **비권장:** `대법원 2008. 1. 10. 선고 2006두19105 판결`
            (이유: 여러 패턴 동시 매칭으로 정확도 저하)
